...
void EventQueue::Add ( Event d, EventTarget* target, uint32_t arrival )
{
    ...
    if (arrival == current->time)
        ...
        assert(current->q.size() == 0); // Checking that we cleared out the 'current' queue
...
/** Clear
 *  removes all leftover elements in the queue and creates new ones with global_time of 0
 *
 */
