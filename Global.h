...
typedef enum {
  ...
  EAST = 2,
  WEST = 3,
  HERE = 4, //!< Indicates current router. VALUE MUST BE 4 FOR Router::Process
            //!TO WORK PROPERLY.
  INVALID = 5
...
typedef enum {
  ...
  BIT_COMP = 2,
  MEM_CONT = 3
} Destination_Function; //!< Defines which function is being used for generating
                        //!destinations
...
extern Router
    *NArray; //!< Array of Routers corresponding to the request network
extern Router
    *NArray2; //!< Array of Routers corresponding to the response network
