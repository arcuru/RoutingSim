...
Packet::Packet(Address destination, Address origin, size_t packet_size) {
  ...
  header |= uint32_t(destination.y) << 3;
  header |= uint32_t(origin.x) << 6;
  header |= uint32_t(origin.y) << 9;
 ...
 */
Flit *Packet::GetFlit(size_t index) const { return &flits[index]; }

/** GetSize
 *  returns the size of the packet in flits
 */
size_t Packet::GetSize() const { return flit_count; }

/** GetX
 *  returns the value of the destination x address stored in the system
 *
 */
uint8_t Packet::GetX() const { return flits[0].getData() & 0x7; }

/** GetY
 *  returns the value of the destination y address stored in the system
 *
 */
uint8_t Packet::GetY() const { return (flits[0].getData() >> 3) & 0x7; }

/** GetOriginX
 *  returns the value of the originating x address stored in the system
 *
 */
uint8_t Packet::GetOriginX() const { return (flits[0].getData() >> 6) & 0x7; }

/** GetOriginY
 *  returns the value of the originating y address stored in the system
 *
 */
uint8_t Packet::GetOriginY() const { return (flits[0].getData() >> 9) & 0x7; }

/** GetHash
 *  returns the saved hash value of the data stored in the flits
 *
 */
uint8_t Packet::GetHash() const { return (flits[0].getData() >> 20) & 0xFF; }

/** GetCreated
 *  returns the creation time of this packet
 *
 */
uint32_t Packet::GetCreated() const { return created; }
 ...
 */
void Packet::SetCreated(uint32_t creation) { created = creation; }
