 ...
 */
InputBuffer *PacketGen::GetInjection() const { return ibuf; }
 ...
 */
OutputBuffer *PacketGen::GetEjection() const { return obuf; }
...
void PacketGen::GenPacket() {
  ...
  uint8_t tmp[] = {0x0, 0x4, 0x2, 0x6,
                   0x1, 0x5, 0x3, 0x7}; // Lookup table for 3bit BIT_REV

  // Generate destination address based on the destination function of the
  // network
  Address dest = addr;
  ...
  case MEM_CONT:
    dest = MC[size_t(rand()) % (sizeof(MC) / sizeof(MC[0]))];
    break;
  }
...
void PacketGen::RandomGenPacket(double chances) {
  ...
  } else if (saved_p->GetCreated() <
             Global_Time -
                 3) // Increment if this packet had the chance to leave by now.
    if (rand() <
        (chances *
         (double)RAND_MAX)) // And if we would have passed our chance test
      packet_injections++;
  return;
}
...
void PacketGen::Process() {
  ...
  if (vc->FlitsRemaining() != 0) {
    if (vc->GetPacket()->GetSize() == vc->FlitsRemaining()) {
      ...
      // cout << "Got full packet at (" << addr.x << ", " << addr.y << ")" <<
      // endl;
      ...
      if (memcontroller) {
        ...
        dest.y = p->GetOriginY();
        Packet *pp =
            NArray2[IND(addr.x, addr.y)].GetPacketGen()->InsertPacket(dest, 68);
        if (nullptr != pp) {
