 ...
 */
InputBuffer *Router::GetTarget(Direction edge) const { return ibuf[edge]; }
 ...
 */
PacketGen *Router::GetPacketGen() const { return pgen; }
...
void Router::Process() {
  ...
  for (int j = 2; j >= 0; j--) {
    for (size_t k = 0; k < 5; k++) {
      ...
      OutputChannel *vc = obuf[i]->getOC(size_t(j));
      if (vc->FlitsRemaining() == vc->Size()) {
        ...
        InputChannel *b = RC->getNext(i, size_t(j));
        if (b != nullptr) {
          // Valid VC exists
