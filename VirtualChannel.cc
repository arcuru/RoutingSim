 ...
 */
uint8_t VirtualChannel::Size() const { return buf_size; }
...
void VirtualChannel::PopFlit() {
  ...
  if (buf_valid == buf_index) {
    // Save something invalid to buf_valid so that we can check if there is
    // valid data
 ...
 */
uint8_t VirtualChannel::FlitsRemaining() const { return buf_flitcount; }
 ...
 */
Packet *VirtualChannel::GetPacket() const { return cur_packet; }
